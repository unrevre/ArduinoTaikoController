...
/*
 *  Adapted from
 *  https://github.com/progmem/Switch-Fightstick/blob/master/Joystick.c
 */
...
Joystick_::Joystick_() {
    ...
    /* This descriptor needs to be the first one so that the device can be */
    /* recognized by the Switch. */
    HID().PrependDescriptor(&node);

    data.BTN = 0;
    data.HAT = SWITCH_HAT_CENTER;
    data.LX = 128;
    data.LY = 128;
    data.RX = 128;
    data.RY = 128;
...
void Joystick_::report(uint16_t state) {
    data.BTN = state;

    /* Not sending report ID. Switch doesn't deal with that properly. */
    HID().SendRaw(&data, 8);
